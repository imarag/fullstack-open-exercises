...
const App = () => {
  ...
  async function handleAddLike(blogId) {
    try {
      ...
      const updatedBlog = await blogService.updateBlog(blogId, blogToUpdate)
  ...
  function handleSortBlogs(mode) {
    ...
    sortedBlogs.sort((a, b) => mode === 'asc' ? a.likes - b.likes : b.likes - a.likes)
        ...
        user && (
          <div>
            <div>
              ...
              <button onClick={() => handleSortBlogs('asc')}>sort by likes ascending</button>
              <button onClick={() => handleSortBlogs('desc')}>sort by likes descending</button>
              {blogs.map(blog =>
                <Blog key={blog.id} currentUser={user} blog={blog} handleAddLike={() => handleAddLike(blog.id)} handleRemoveBlog={() => handleRemoveBlog(blog.id)} />
              )}
            </div>
